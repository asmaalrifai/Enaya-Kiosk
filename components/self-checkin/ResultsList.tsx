...
export default function ResultsList({
  ...
  const isFullPhone = PHONE_REGEX.test(query);
  ...
  return (
    <>
      ...
      <ul className="mt-2 max-h-72 overflow-auto divide-y" style={{ borderColor: `${BRAND_DARK}10` }}>
        {/* Show "no match" ONLY when a full phone number has been entered and there are no results */}
        {isFullPhone && results.length === 0 && !loading && (
        ...
        {/* Render results (these appear only when full number used since that's when we fetch) */}
        {results.map((c) => (
          <li key={c.id}>
            <button
              className="w-full text-left p-3 hover:bg-gray-50 focus:bg-gray-50 transition flex items-center gap-3"
              onClick={() => onSelect(c)}
            >
              <div
              ...
              <div className="flex-1">
                ...
                <div className="text-xs opacity-80" style={{ color: BRAND_DARK }}>
                  {maskPhone(c.phone)}
                  {c.membership ? ` • ${c.membership} ${strings.member}` : ""}
                </div>
              </div>
              <div
